...
export const L10nRegistry = {
  ...
  * generateContexts(requestedLangs, resIds, pkg = 'main') {
    for (const loc of requestedLangs) {
      yield * generateContextsForLocale(pkg, loc, L10nRegistry.sourcesOrder, resIds);
    }
  },
...
function* generateContextsForLocale(pkg, loc, sourcesOrder, resIds, resolvedOrder = []) {
  ...
  for (const sourceId of sourcesOrder) {
    if (L10nRegistry.sources.get(sourceId).packages.length > 0 &&
      !L10nRegistry.sources.get(sourceId).packages.includes(pkg)) {
      continue;
    }
    ...
    if (resolvedLength + 1 === resourcesLength) {
      ...
      // list of sources.
      yield * generateContextsForLocale(pkg, loc, sourcesOrder, resIds, order);
    }
  }
}
